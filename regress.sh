...
cat build/$variant/kernal/kernal.bin /dev/zero | dd bs=1 count=24832 skip=19840 >> build/compare/kernal.bin 2> /dev/null
...
for file in kernal reu0 reu1 reu2 reu3 reu4 reu5 reu6 reu7 reu8 reu9 reu10 reu11 reux; do
	if [ -e build/compare/$file.bin ]; then
		echo $file
		hexdump -C build/compare/$file.bin > build/compare/$file.txt
		hexdump -C build/compare/${file}_reference.bin > build/compare/${file}_reference.txt
		diff --suppress-common-lines -y build/compare/${file}_reference.txt build/compare/${file}.txt | head -n 10
	fi
done
